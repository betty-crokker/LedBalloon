...
async Task<int> RunAsync(string[] argv, CancellationToken ct)
{
    ...
    if (argv.Length < 2)
    {
        Error($"'{command}' needs a device host, e.g. ledballoon {command} 192.168.1.50");
        return 2;
    }
...
async Task<int> BrightnessAsync(WledClient client, string[] rest, CancellationToken ct)
{
    ...
    {
        Error("Usage: ledballoon bri <host> <0-255>");
        return 2;
    }
...
async Task<int> ColorAsync(WledClient client, string[] rest, CancellationToken ct)
{
    ...
    {
        Error("Usage: ledballoon color <host> <#rrggbb> [segment]");
        return 2;
    }
...
async Task<int> EffectAsync(WledClient client, string[] rest, CancellationToken ct)
{
    if (rest.Length == 0)
    {
        Error("Usage: ledballoon effect <host> <index or name> [segment]");
        return 2;
    }
    ...
    else
    {
        ...
        if (index < 0)
        {
            Error($"No effect matching '{rest[0]}'. Run: ledballoon effects <host>");
            return 1;
        }
    }
...
async Task<int> ApplyPresetAsync(WledClient client, string[] rest, CancellationToken ct)
{
    ...
    {
        Error("Usage: ledballoon preset <host> <slot>");
        return 2;
    }
...
async Task<int> AuditAsync(WledClient client, string host, CancellationToken ct)
{
    ...
    Console.WriteLine("  Re-save them from LedBalloon to refit them to the strip as it is now.");
...
async Task<int> RenameAsync(string host, string[] rest, CancellationToken ct)
{
    if (rest.Length == 0)
    {
        Error("Usage: ledballoon rename <host> <new name>");
        return 2;
    }
...
int Photo(string[] rest)
{
    if (rest.Length == 0)
    {
        Error("Usage: ledballoon photo <file> [budget KB]");
        return 2;
    }
...
{
    Console.WriteLine("""
        ledballoon - a command-line WLED client

        USAGE
          ledballoon <command> [host] [args]
        ...
        EXAMPLES
          ledballoon discover
          ledballoon audit 192.168.1.50
          ledballoon color wled-a1b2c3.local "#ff6600"
          ledballoon effect 192.168.1.50 twinkle 1
        """);
}
